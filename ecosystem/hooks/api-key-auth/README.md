...
API key authentication hook factory for burger-api framework. Hook factories are code that runs around your handler — before and/or after it. This hook protects your routes by verifying API keys from headers or query parameters.
...
```typescript
import { apiKeyWithRateLimit } from './ecosystem/hooks/api-key-auth/api-key-auth';
