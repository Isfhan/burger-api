...
JWT (JSON Web Token) authentication hook factory for burger-api framework. Hook factories are code that runs around your handler — before and/or after it. This hook verifies JWT tokens from requests and protects your routes from unauthorized access.
...
```typescript
// api/users/profile/hooks.ts
import { jwt } from '../../../ecosystem/hooks/jwt-auth/jwt-auth';

export const beforeHandle = [
    jwt({ secret: process.env.JWT_SECRET! }), // Only protect this route
];
```
