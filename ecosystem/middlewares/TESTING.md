...
mkdir -p ecosystem/middlewares

# Copy the middleware you want to test
# For example, to test CORS:
cp -r /path/to/burger-api/middlewares/cors ./ecosystem/middlewares/

# Or copy all middleware:
cp -r /path/to/burger-api/middlewares/* ./ecosystem/middlewares/
```
...
import {
    securityHeaders,
    strictSecurity,
} from './ecosystem/middlewares/security-headers/security-headers';
...
```typescript
import { jwt, createJWT } from './ecosystem/middlewares/jwt-auth/jwt-auth';

const SECRET = 'test-secret-key-12345';

const app = new Burger({
    apiDir: './api',
    globalMiddleware: [jwt({ secret: SECRET })],
});
```
...
```typescript
import { publicCache, noCache } from './ecosystem/middlewares/cache/cache';
...
-   **Solution**: Make sure middleware is in `globalMiddleware` array or
