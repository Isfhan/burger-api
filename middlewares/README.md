...
A comprehensive collection of production-ready middleware for the burger-api framework. Each middleware is designed to be easy to use, highly configurable, and follows best practices for web application security and performance.
...
This middleware collection requires **Bun.js runtime v1.3.1+** and the burger-api framework:
...
Each middleware is in its own subfolder. To use a middleware, copy the folder to your project:

```bash
# Coming soon: CLI tool for easy installation
burger-api add cors
burger-api add rate-limiter
burger-api add logger

# Or manually copy the middleware folders you need
```
...
import { rateLimit } from './middlewares/rate-limiter/rate-limiter';
...
Burger API middleware follows a unique pattern that's different from Express or Koa:
...
For comprehensive manual testing of all middleware in a real Bun.js environment, see **[TESTING.md](./TESTING.md)**.
...
All middleware are optimized for **Bun.js runtime v1.3.1+** with automatic fallbacks for other runtimes.

### Performance Improvements on Bun v1.3.1:
- **Logger**: Uses `Bun.nanoseconds()` for microsecond-precision timing (**3x faster**)
- **Rate Limiter**: Uses `Bun.CryptoHasher` for ultra-fast key hashing (**10x faster**)
- **JWT Auth**: Native `crypto.subtle` implementation (**2x faster** than Node.js)
...
### Known Limitations:
- ⚠️ **Brotli compression** not yet supported in Bun's `CompressionStream` (use `gzip` or `deflate`)
...
**No configuration needed** - optimizations activate automatically when running on Bun!
...
*Benchmarks are approximate and vary based on payload size and hardware.*
