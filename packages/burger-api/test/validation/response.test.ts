...
import type { CompiledRouteValidators, ValidatorConfig } from '../../src/validation/types';
                ...
                '200': {
                    ...
                    validate: (v: unknown) => {
                        ...
                        return r.success
                            ...
                            : { success: false, issues: r.error.issues.map((i) => ({
...
describe('ResponseValidator (M5)', () => {
    it('returns ok when no response schema exists', () => {
        const v = { methods: {} } as CompiledRouteValidators;
        expect(validateResponse(v, 'get', 200, { ok: true }, {}, false).ok).toBe(true);
    });
    ...
    it('dev mode: mismatch logs + passes through (ok)', () => {
        ...
        const out = validateResponse(v, 'get', 200, { ok: 'not-boolean' }, { responseValidation: 'dev' }, true);
        expect(out.ok).toBe(true);
    });

    it('enforce mode: mismatch returns safe 500 (no internals)', () => {
        const v = makeValidators();
        const out = validateResponse(v, 'get', 200, { ok: 'nope' }, { responseValidation: 'enforce' }, false);
    ...
    it('enforce mode: valid response passes', () => {
        const v = makeValidators();
        const out = validateResponse(v, 'get', 200, { ok: true }, { responseValidation: 'enforce' }, false);
        expect(out.ok).toBe(true);
    });

    it('off mode: mismatch passes through', () => {
        const v = makeValidators();
        const out = validateResponse(v, 'get', 200, { ok: 'nope' }, { responseValidation: 'off' }, false);
        expect(out.ok).toBe(true);
    });
    ...
    it('does not apply another method response schema', () => {
        ...
        const out = validateResponse(v, 'post', 200, { ok: 'nope' }, { responseValidation: 'enforce' }, false);
        expect(out.ok).toBe(true);
    });
});
...
function stubAlwaysFail() {
    return {
        ...
        slot: 'body' as const,
        identity: 'fail',
        validate: () => ({ success: false, issues: [{ path: [], message: 'x' }] }),
        coercible: false,
    };
}
